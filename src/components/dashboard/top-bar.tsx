...
export function TopBar({ userEmail, brandId, walletBalance = 0 }: TopBarProps) {
      ...
      <header data-print-hide className="fixed top-0 right-0 left-0 md:left-60 z-20 h-14 glass-panel border-b border-white/[0.06] flex items-center justify-between px-4 md:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="hidden sm:flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#059669]/10 text-[#059669] text-xs font-medium hover:bg-[#059669]/20 transition-colors duration-150"
          >
            <CreditCard className="h-3 w-3" aria-hidden="true" />
            Penny&apos;s Wallet: {walletBalance.toLocaleString()}
          </Link>
